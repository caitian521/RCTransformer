...
description = 'Preprocessor for summarization dataset. Include unsupervised text tokenization and vector word' \
              ...
              '/data/ct/abs_summarize/sumdata/transf_summ/dataset    /data/ct/abs_summarize/sumdata/transf_summ/models_dumps'

parser = argparse.ArgumentParser(description=description, formatter_class=argparse.ArgumentDefaultsHelpFormatter)

parser.add_argument('--dataset', metavar='DIR', type=str, default='/data/ct/abs_summarize/sumdata/transf_summ/dataset', help='dataset directory')
parser.add_argument('--vocab_size', metavar='V', type=int, default=15000, help='vocabulary size')
...
parser.add_argument('--prefix', metavar='P', type=str, default='bpe-summ', help='model prefix')

args = parser.parse_args()
train_filename = os.path.join(args.dataset, 'train.tsv')
test_filename = os.path.join(args.dataset, 'test.tsv')
sp_model_prefix = os.path.join('/data/ct/abs_summarize/sumdata/transf_summ', args.prefix, args.prefix + '_bpe')
sp_model_filename = sp_model_prefix + '.model'
w2v_model_filename = os.path.join('/data/ct/abs_summarize/sumdata/transf_summ', args.prefix, 'word2vec.model')
embeddings_filename = os.path.join('/data/ct/abs_summarize/sumdata/transf_summ', args.prefix, 'embedding.npy')

for path in [sp_model_filename, w2v_model_filename, embeddings_filename]:
    os.makedirs(os.path.dirname(path), exist_ok=True)

# Start tokenization training:
spm_params = '--pad_id=0 --unk_id=1 --bos_id=2 --eos_id=3 --model_type=BPE ' \
